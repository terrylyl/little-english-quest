...
export type MiniGameType = 'listen-picture' | 'match' | 'memory' | 'sort';
...
export type LessonState = {
  ...
  isRecording: boolean; canSkipSpeaking: boolean; speakingOutcome: SpeakingOutcome; speakAttempts: number;
  gameType: MiniGameType; activityResult: ActivityResult; mistakes: number;
};
...
export function createLessonState(themeId: ThemeId, level: LevelNumber, words: WordEntry[], warmUpWords: WordEntry[] = [], listenOptionCount: 2 | 4 = 4, random: () => number = Math.random): LessonState {
  ...
  const games: MiniGameType[] = ['listen-picture', 'match', 'memory', 'sort'];
  return { themeId, level, words: lessonWords, warmUpWords, listenOptions: shuffled(lessonWords, random).slice(0, Math.min(listenOptionCount, lessonWords.length)), step: warmUpWords.length ? 'warm-up' : 'learn', promptWord, feedback: 'idle', isRecording: false, canSkipSpeaking: false, speakingOutcome: null, speakAttempts: 0, gameType: games[Math.floor(random() * games.length)], activityResult: { completed: false, correctCount: 0, mistakeCount: 0, practicedWordIds: lessonWords.map((word) => word.id) }, mistakes: 0 };
}
...
export function advanceLesson(state: LessonState): LessonState {
  ...
  return { ...state, step: nextStep[state.step], feedback: 'idle', isRecording: false };
}
export function answerListenPrompt(state: LessonState, wordId: string): LessonState { if (state.step !== 'listen') return state; const correct = wordId === state.promptWord.id; return { ...state, feedback: correct ? 'correct' : 'try-again', mistakes: state.mistakes + (correct ? 0 : 1) }; }
export function completeActivity(state: LessonState, correct: boolean): LessonState { if (state.step !== 'play') return state; return { ...state, activityResult: { ...state.activityResult, completed: correct, correctCount: state.activityResult.correctCount + (correct ? 1 : 0), mistakeCount: state.activityResult.mistakeCount + (correct ? 0 : 1) }, mistakes: state.mistakes + (correct ? 0 : 1), feedback: correct ? 'correct' : 'try-again' }; }
...
export function allowSpeakingSkip(state: LessonState): LessonState { return state.step === 'speak' ? { ...state, isRecording: false, canSkipSpeaking: true } : state; }
export function skipSpeaking(state: LessonState): LessonState { return state.step === 'speak' && state.canSkipSpeaking ? { ...state, speakingOutcome: 'skipped', isRecording: false } : state; }
