...
import { advanceLesson, answerListenPrompt, completeActivity, completeSpeaking, createLessonState, getLessonStepPosition, getLessonSteps, getRoundPosition, isLastRound, isRoundCleared, isRoundStep, skipSpeaking, startSpeaking, stopSpeaking, type LessonState } from '../domain/lesson';
import { LEVEL_STAR_REWARD } from '../domain/progress';
...
export function LessonScreen({ theme, level, words, warmUpWords, policy, player, onBack, onComplete }: Props) {
  ...
  if (lesson.step === 'reward') return <section className="screen reward-screen" aria-labelledby="reward-title"><div className="reward-badge" aria-hidden="true">★</div><p className="eyebrow">Level {level} complete · +{LEVEL_STAR_REWARD} stars</p><h1 id="reward-title">Quest complete!</h1><p className="intro">{lesson.words.length} new stickers for your album.</p><ul className="reward-stickers" aria-label={`New stickers: ${lesson.words.map((word) => word.word).join(', ')}`}>{lesson.words.map((word, index) => <li className="reward-sticker" key={word.id} style={{ '--sticker-delay': `${index * 90}ms` } as CSSProperties}><img src={word.image} alt="" loading="lazy" /><span>{word.word}</span></li>)}</ul><button className="primary-action" type="button" onClick={() => onComplete(lesson)}>Collect rewards</button></section>;
  const lessonSteps = getLessonSteps(lesson);
  const stepPosition = getLessonStepPosition(lesson);
  const roundPosition = getRoundPosition(lesson);
  const roundMeter = isRoundStep(lesson.step) && <div className="round-meter" aria-hidden="true">{lesson.words.map((word, index) => <span className={index < roundPosition.current - 1 || (index === roundPosition.current - 1 && isRoundCleared(lesson)) ? 'is-done' : index === roundPosition.current - 1 ? 'is-current' : ''} key={word.id} />)}</div>;
